...
/**
 * Listado de prendas de la bolsa, con la foto del producto.
 * Las imágenes del correo necesitan dirección absoluta: un correo no tiene
 * "carpeta actual" desde la que resolver rutas relativas.
 */
function tablaItems(items) {
  const filas = items.map((i) => {
    const foto = i.foto ? `${SITIO}/img/productos/${i.foto}` : null;
    return `
    <tr>
      ${foto ? `<td width="64" style="padding:11px 12px 11px 0;border-bottom:1px solid ${M.filete};">
        <img src="${foto}" width="64" height="80" alt="${i.n}" border="0"
             style="display:block;width:64px;height:80px;object-fit:cover;border:1px solid ${M.filete};">
      </td>` : ""}
...
export function correoPedidoConfirmado({ referencia, lineas, total, nombre, envio }) {
  const items = lineas.map((l) => ({
    n: l.nombre, color: l.color, talla: l.talla, q: l.cantidad, precio: l.precio, foto: l.foto
  }));
